...
class Mel2Audio:
    ...
    def make_audios(self, sample_info, original_audio, startpoint=None, num_concepts=4, percentile=50, path_to_sample=None):
        r"""
        Controls the generation, volume adjustment and normalization of all relevance-audios.
        -----
        Args:
            sample_info (dict): Dict (as obtained with HeatmapGenerator from cxai.xai.explain.explainer). Contains relevance heatmaps.
            original_audio    ()
        """

        assert original_audio is not None or path_to_sample is not None, 'please provide either an audio sample or path to audio file'
        ...
        wav_standard_R = peak_normalizer(torch.tensor(self.transform(sample_info['standard_heatmap'], mel, phase, percentile=50)))
        ...
        for subspace_idx in range(num_concepts):
            ...
            wav_subspace_k = self.transform(sample_info['subspace_heatmaps'][subspace_idx:subspace_idx+1], mel, phase, percentile=percentile)
            # adjust volume to original audio
            audios.append(adjust_vol(original_audio, peak_normalizer(torch.tensor(wav_subspace_k))))
